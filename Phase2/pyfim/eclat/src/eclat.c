...
#define VERSION     "version 5.17 (2016.10.15)        " \
...
{                               /* --- eclat recursion with tid lists */
  ...
  if (rd->mode & ECL_TAIL) {    /* if to use tail to prune w/ repo. */
    t = isr_buf(rd->report);    /* get the item set buffer */
    for (m = 0, i = k; --i > 0; )
...
{                               /* --- eclat with trans. id lists */
  ...
  &&  (l->supp >= rd.smin)) {   /* and there are packed items */
    ...
    for (p = l->tids; *p >= 0; p++)
      l->item |= rd.btas[*p];   /* collect the occurring items and */
  }                             /* add list to the reduced array */
  ...
  if (m > 0) {                  /* if there are frequent items */
    ...
    r = (mode & ECL_EXTCHK)     /* dep. on how to filter closed/max. */
      ? rec_tcm(lists, m, z, 0, &rd)
      : rec_tid(lists, m, z,    &rd);
